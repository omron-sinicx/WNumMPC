...
def objective(nav_config: NavConfig, optuna_trial: optuna.Trial) -> float:
...
def optimize(nav_config: NavConfig, study_name: str, storage: str, trial_num: int) -> None:  # 並列化用の関数
    optuna_study: optuna.Study = optuna.create_study(
        study_name=study_name,
        storage=storage,
        load_if_exists=True,
    )
    obj = lambda x: objective(nav_config, x)
...
def optimize_param(mpc_param: str, train_param: str) -> None:
    ...
    assert wmpc_config.sim.robot_policy == "normal_mpc"
    #wmpc_config.eval_episodes = 30  # for time saving
    #wmpc_config.eval_states = "./datas/eval_states/optuna-{}-30.npy".format(wmpc_config.sim.human_num+1)
    ...
    if not nav_config.policy_config["multi_thread"]:
        ...
        obj = lambda x: objective(copy.deepcopy(nav_config), x)
        study.optimize(func=obj, n_trials=n_trials)

    else:  # 並列化
        concurrency = min(mp.cpu_count(),36)  # max_cpuより使うCPUの数が多くないことを確認
        ...
        # 並列化
        workers = [mp.Process(target=optimize, args=(nav_config, study_name, DATABASE_URI, n_trials_per_cpu)) for _ in
                   range(concurrency)]
...
if __name__ == '__main__':
    ...
    mpc_param: str = "rot_real_normal_mpc_H{}_margin".format(human_num)
    optimize_param(mpc_param, "default")
