...
class CADRL(Policy):
    def __init__(self, config: Config, agent_id: int):
        ...
        self.configure(config.policy_config)

    def configure(self, config: DictConfig):
        self.set_common_parameters(config.cadrl_param)
        mlp_dims = config.cadrl.mlp_dims
        self.model = ValueNetwork(self.joint_state_dim, mlp_dims)
        self.multiagent_training: bool = config.cadrl.multiagent_training

    def set_common_parameters(self, config: DictConfig):
    ...
    def predict(self, state: JointState):
        """
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                ob_pre, reward = self.env.onestep_lookahead(action, agent_id=self.id)
                ob = ob_pre[1:]
    ...
    def transform(self, state: JointState) -> torch.Tensor:
        """
        ...
        concat_state = torch.cat([torch.Tensor([state.self_state + h_state]).to(self.device) for h_state in state.human_states], dim=0)
        state = self.rotate(concat_state).squeeze(dim=0)
        return state
