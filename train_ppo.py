    ...
    for i in tqdm(range(n_iter), desc="[TRAIN]"):  # training loop
        ...
        if (writer is not None and i % eval_span == 0) or (i == (n_iter - 1)):
            ...
            if logging:
                ...
                if rew_lambda != 0.0:
                    model_path = "./models/ww_human_{}/diff_wheel/{}_lambda{}".format(base_env.human_num, model_name, int(rew_lambda*100))
                else:
                    model_path = "./models/ww_human_{}/diff_wheel/{}".format(base_env.human_num, model_name)
